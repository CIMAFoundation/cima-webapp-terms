...
export class UploadPageComponent {
  ...
  get canPublish(): boolean {
    return this.uploadFacade.canPublish();
  }
  ...
  onFilesSelected(files: FileList | null): void {
    const result = this.uploadFacade.addFiles(files, {
      line: '',
      lang: '' as '' | UploadLanguage,
      docType: '' as '' | UploadDocType,
  ...
  async uploadAll(): Promise<void> {
    ...
    if (!this.canPublish) {
      this.showSnackbar('Compila metadati per ogni file e seleziona almeno un PDF', 'error');
      return;
    }
